...
class DBHelper private constructor(val context: Context) : SQLiteOpenHelper(context, DB_NAME, null, DB_VERSION) {
    ...
    private val COL_DISTACIA = "distancia"
    ...
    override fun onCreate(db: SQLiteDatabase) {
        db.execSQL("CREATE TABLE $ALARMS_TABLE_NAME ($COL_ID INTEGER PRIMARY KEY AUTOINCREMENT, $COL_TIME_IN_MINUTES INTEGER, $COL_DAYS INTEGER, " +
                "$COL_IS_ENABLED INTEGER, $COL_VIBRATE INTEGER, $COL_SOUND_TITLE TEXT, $COL_SOUND_URI TEXT, $COL_LABEL TEXT, $COL_LATITUDE DOUBLE," +
                " $COL_LONGITUDE DOUBLE, $COL_DISTACIA DOUBLE)")
    ...
    private fun fillAlarmContentValues(alarm: Alarm): ContentValues {
        return ContentValues().apply {
            ...
            put(COL_DISTACIA, alarm.distancia)
            put(COL_LONGITUDE, alarm.longitude)
            put(COL_LATITUDE, alarm.latitude)
        }
    }
    ...
    fun getAlarms(): ArrayList<Alarm> {
        ...
        val cols = arrayOf(COL_ID, COL_TIME_IN_MINUTES, COL_DAYS, COL_IS_ENABLED, COL_VIBRATE, COL_SOUND_TITLE, COL_SOUND_URI, COL_LABEL, COL_LATITUDE, COL_LONGITUDE, COL_DISTACIA)
            ...
            if (cursor?.moveToFirst() == true) {
                do {
                    try {
                        ...
                        val distancia = cursor.getStringValue(COL_DISTACIA)

                        val alarm = Alarm(id, timeInMinutes, days, isEnabled, vibrate, soundTitle, soundUri, label, latitude.toDouble(), longitude.toDouble(), distancia.toDouble())
                        alarms.add(alarm)
                    } catch (e: Exception) {
                        continue
                    }
